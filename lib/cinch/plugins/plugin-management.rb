...
module Cinch; module Plugins; class PluginManagement
  include Cinch::Plugin

  match(/plugin load (\S+)(?: (\S+))?/i, method: :load_plugin)
  match(/plugin unload (\S+)/i, method: :unload_plugin)
  match(/plugin reload (\S+)(?: (\S+))?/i, method: :reload_plugin)
  ...
  def load_plugin(m, plugin, mapping)
    ...
    registered_plugin = @bot.plugins.register_plugin(const).last
    registered_plugin.timers.each { |timer| timer.start }
